...
export default function GameScreen6({ route, navigation }:any) {
  ...
  const responder = (correta:boolean) => {
    ...
    navigation.navigate('Game8', { nome, pontos: novosPontos });
  };

  return (
    <View style={styles.container}>
      {/* Título mantido como na Home */}
      <Text style={styles.title}>LEVEL 07</Text>

      {/* Caixa do Arcade envolvendo a pergunta e respostas */}
      <View style={styles.divArcade}>
        <Text style={styles.question}>Por que saíram no meio da formatura?</Text>

        {["Fizeram shit in the pants", "Foram embora", "Sairam comer", "Foram ao banheiro"].map((opcao, index) => (
          <TouchableOpacity
            key={index}
            style={styles.card}
            onPress={() => responder(opcao === "Fizeram shit in the pants")}
          >
